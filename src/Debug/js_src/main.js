...
var PHPDebugConsole = (function($, clipboard) {
	...
	*/
	...
	function addPersistOption($root) {
		...
		if (options.debugKey) {
			...
			if (module.cookie.get("debug") === options.debugKey) {
			...
			$("input", $node).on("change", function() {
				...
				if (checked) {
					...
					module.cookie.save("debug", options.debugKey, 7);
				} else {
					module.cookie.remove("debug");
				}
			});
	...
	function getDebugKey() {
		var key = null,
			queryParams = module.cookie.queryDecode(),
			cookieValue = module.cookie.get("debug");
