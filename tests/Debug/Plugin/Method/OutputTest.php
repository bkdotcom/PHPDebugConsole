...
class OutputTest extends DebugTestFramework
{
    ...
    public function testOutputEvent()
    {
        ...
        $this->debug->eventManager->subscribe(Debug::EVENT_OUTPUT, static function (Event $event) use (&$called) {
            $channelName = $event->getSubject()->getCfg('channelName', Debug::CONFIG_DEBUG);
            $called[] = $channelName . ' ' . \json_encode($event['isTarget']);
            $event['return'] .= ' ' . $channelName;
        });
        ...
        // Debug::varDump('return', $return);
        self::assertStringMatchesFormat('<div class="debug" data-channel-name-root="general.foo"%A</div>%Ageneral.foo', $return);
